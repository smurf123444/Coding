...
void showq(queue <Employee> gq) 
{ 
    queue <Employee> g = gq; 
    while (!g.empty()) 
    { 
        g.front().printEmployee(); 
        g.pop(); 
    } 
    cout << '\n'; 
} 
...
int main() 
{ 
    queue <Employee> gquiz; 
    Employee test, test1, test2,temp, temp1, temp2;
    temp = test.writeEmployee("245454","Manning","Chris","24",'m');
    temp1 = test.writeEmployee("234343","Ron","Fweag","23",'f');
    temp2 = test.writeEmployee("245454","Howard","Read","21",'m');
    gquiz.push(temp); 
    gquiz.push(temp1); 
    gquiz.push(temp2); 
    ...
    cout << endl;
    gquiz.front().printEmployee(); 
    gquiz.back().printEmployee(); 
